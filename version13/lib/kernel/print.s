...
global put_int   ;16进制打印 比如传入0x00123打印123
put_int:
   pushad
   mov ebp,esp
   mov eax,[ebp+36];8个寄存器和一个返回值。参数地址
   mov edx,eax
   mov edi,7   ;buffer指针
   mov ecx,8   ;循环次数
...
.16based_4bits:
   ...
   cmp edx,9
   jg .is_A2F
   add edx,'0'   ;和字符'0' add操作后 保留低4位有效
   jmp .store
.is_A2F:
   sub edx,10
   add edx,'A'
...
.store:
   mov [ebx + edi],dl;将转后的ascii码值存到缓冲buffer中
   dec edi
   shr eax,4 ;下一个4位
   mov edx,eax
   loop .16based_4bits
...
.ready_to_print:
   inc edi ;初始化新变量 循环次数 和 buffer指针变量
.skip_prefix_0:
   cmp edi,8
   je .full0
.go_on_skip:
